...
class MLP(nn.Module):
    def __init__(self):
        ...
        self.mlp_stack = nn.Sequential(            
            ...
            nn.ReLU(), # ativar saída - transformar entre 0 e x
            nn.Linear(5, 3),  # terceira camada - aplicar transformação de linear. 2 neurônios (2 categorias)
...
def _get_data_loader(batch_size, X, y, is_distributed, **kwargs):
    ...
    dataset = CustomDataset(torch.from_numpy(X).float(), torch.from_numpy(y).long())
